...
/**
 * Async webhook flow: Progress after 2s, then Success or Failure after 3s.
 * ---------------------------------------------------------------------------
 * Simulates real VIM behaviour where the infrastructure reports progress and
 * then completes (or fails). Random Success/Failure demonstrates compensation
 * and NFVO notification when the LCM receives InfraDeploymentFailedEvent.
 */
@Slf4j
@Service
...
public class DeployWebhookService {
    ...
    private void postProgress(String callbackUrl, String vnfId, String deploymentId) {
        ...
        try {
            ...
            log.info("Progress webhook sent: vnfId={}", vnfId);
        } catch (Exception e) {
    ...
    private void postSuccess(String callbackUrl, String vnfId, String deploymentId) {
        ...
        try {
            restTemplate.postForObject(callbackUrl, body, String.class);
            log.info("Success webhook sent: vnfId={}", vnfId);
        } catch (Exception e) {
    ...
    private void postFailure(String callbackUrl, String vnfId, String deploymentId) {
        Map<String, Object> body = Map.of(
                ...
                "deploymentId", deploymentId != null ? deploymentId : "",
                "type", "FAILURE",
                "reason", "Simulated failure (quota or image not found)",
                "errorCode", "SIM_FAIL"
        );
        try {
            restTemplate.postForObject(callbackUrl, body, String.class);
            log.info("Failure webhook sent: vnfId={}", vnfId);
        } catch (Exception e) {
